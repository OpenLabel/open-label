...
import { validateCarCleaning, publicCarCleaningData } from '@/lib/carCleaning';
...
describe('car cleaning sample integrity', () => {
  it('provides a complete current-profile fictitious cleaner without a certification claim', () => {
    const build = getSamplePassport('car_cleaning');
    expect(build).toBeTypeOf('function');
    const sample = build!();
    const data = sample.category_data as Record<string, unknown>;

    expect(sample.category).toBe('car_cleaning');
    expect(data.dpp_profile).toBe('current');
    expect(validateCarCleaning(data)).toEqual([]);
    expect(publicCarCleaningData(data).product_name).toBe('Sample Car Shampoo');
    expect(sample.description).toMatch(/fictitious/i);
    expect(sample.description).toMatch(/not a certification/i);
    expect(data.manufacturer_email).toBe('contact@example-cleaner.example');
    expect(data.ingredients_url).toBe('https://example-cleaner.example/ingredients');
  });
});
