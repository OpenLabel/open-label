...
interface GarmentAIAutofillProps {
  onAutofill: (data: Record<string, unknown>) => void;
  onAutofillMeta?: (meta: { dppName?: string; productImageBase64?: string }) => void;
}

export function GarmentAIAutofill({ onAutofill, onAutofillMeta }: GarmentAIAutofillProps) {
  ...
  const processImage = async (file: File) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('garment-label-ocr', {
        body: { image: base64 },
      });
      ...
      console.error('Error processing garment label:', error);
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Sparkles className="h-5 w-5 text-purple-500" />
              {t('apparel.ai.scannerTitle', t('ai.scannerTitle'))}
            </DialogTitle>
            <DialogDescription>
              {t(
                'apparel.ai.scannerDescription',
                'Photograph or upload the sewn-in care label, the hangtag, or a tech pack PDF — the AI will read the fibre composition, care symbols, made-in country, certifications, and style references.',
              )}
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={showQuotaDialog} onOpenChange={setShowQuotaDialog}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            <p className="text-sm text-muted-foreground">{t('ai.quotaExceededBody')}</p>
            <p className="text-sm text-muted-foreground">{t('ai.quotaContactBody')}</p>
          </div>
          <DialogFooter className="flex-col sm:flex-row gap-2">
            <Button variant="outline" onClick={() => setShowQuotaDialog(false)} className="w-full sm:w-auto">
              {t('common.close')}
            </Button>
            <Button
              onClick={() => {
                window.location.href = 'mailto:contact@open-label.eu?subject=AI%20Garment%20Scan%20Quota%20Increase%20Request';
              }}
