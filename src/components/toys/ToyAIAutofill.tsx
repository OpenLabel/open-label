...
export function ToyAIAutofill({ onAutofill, onAutofillMeta }: ToyAIAutofillProps) {
  ...
  const processImage = async (file: File) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('toy-label-ocr', {
        body: { image: base64 },
      });
      ...
      console.error('Error processing toy packaging:', error);
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Sparkles className="h-5 w-5 text-purple-500" />
              {t('toys.ai.scannerTitle', t('ai.scannerTitle'))}
            </DialogTitle>
            <DialogDescription>
              {t(
                'toys.ai.scannerDescription',
      ...
      <Dialog open={showQuotaDialog} onOpenChange={setShowQuotaDialog}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            ...
            <Button
              onClick={() => {
                window.location.href = 'mailto:contact@open-label.eu?subject=AI%20Toy%20Scan%20Quota%20Increase%20Request';
              }}
